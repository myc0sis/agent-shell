...
With agent-shell, you can chat with the likes of Gemini CLI, Claude Code, NanoCode, Auggie, Mistral Vibe, or any other ACP-driven agent.
...
See https://github.com/nanogpt-community/nanocode for details.
...
(setq agent-shell-nanocode-authentication
      (agent-shell-nanocode-make-authentication :api-key "your-nanogpt-api-key-here"))

;; With function
(setq agent-shell-nanocode-authentication
      (agent-shell-nanocode-make-authentication
       :api-key (lambda () (auth-source-pass-get "secret" "nanogpt-api-key"))))
#+end_src
...
By default, =agent-shell= includes configurations for all supported agents (Claude Code, Gemini CLI, Codex, NanoCode, Goose, Qwen Code, and Auggie). You can customize which agents are available through the =agent-shell-agent-configs= variable.
...
- =M-x agent-shell-nanocode-start-agent= - Start a NanoCode agent session
...
| agent-shell-nanocode-authentication           | Configuration for NanoCode authentication.                                      |
| agent-shell-nanocode-command                  | Command and parameters for the NanoCode client.                                 |
| agent-shell-nanocode-environment              | Environment variables for the NanoCode client.                                  |
...
|                 | agent-shell-nanocode-start-agent                        | Start an interactive NanoCode agent shell.                                    |
...
- [[https://github.com/nanogpt-community/nanocode][NanoCode]].
